...
(defn- portable-test?
  "Keep only deterministic, printable, pure-Clojure tests."
  [{:keys [expr it result error]}]
...
(defn- core-category?
  "Only clojure.core.* categories, not sub-namespaces like clojure.core.reducers.*"
  [cat]
  (let [s (name cat)]
...
(defn emit-cljc [output-path]
  (let [ref (edn/read-string (slurp reference-file))
        tests (filter portable-test? ref)
        by-cat (into (sorted-map) (filter (fn [[k _]] (core-category? k))
