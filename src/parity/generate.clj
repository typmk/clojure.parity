#!/usr/bin/env clojure
;; =============================================================================
;; generate.clj — reflect, generate specs, expand, capture, verify
;;
;; The entire init pipeline: JVM reflection → test specs → expand parametric
;; templates → capture JVM reference answers → verify.
;; =============================================================================
...
(require '[clojure.string :as str]
         ...
         '[clojure.edn :as edn]
         '[clojure.pprint]
         ;; Needed for eval during capture
         '[clojure.set] '[clojure.walk] '[clojure.data] '[clojure.math]
         '[clojure.core.reducers] '[clojure.spec.alpha :as s]
...
(defn expand-parametric-tests [{:keys [describe params template]}]
  (mapv (fn [bindings]
...
(defn expand-category [ns-prefix {:keys [category tests parametric]}]
  (let [full-cat (if ns-prefix
    ...
    (into
      ...
      (mapcat (fn [p]
                (map #(assoc % :category full-cat :ns ns-prefix) (expand-parametric-tests p)))
              parametric))))
...
;; =============================================================================
;; Emit parity.cljc — standalone test file, runs anywhere
;; =============================================================================

;; Layer ordering: tests are emitted root-first so failures cascade naturally.
;; Categories that depend on nothing come first; derived categories come later.
(def layer-order
  [:clojure.core.arithmetic
   :clojure.core.comparison
   :clojure.core.boolean-logic
   :clojure.core.coercion
   :clojure.core.bitops
   :clojure.core.strings
   :clojure.core.lists
   :clojure.core.vectors
   :clojure.core.maps
   :clojure.core.sets
   :clojure.core.sequences
   :clojure.core.lazy-sequences
   :clojure.core.functions
   :clojure.core.metadata
   :clojure.core.transients
   :clojure.core.transducers
   :clojure.core.sorted-collections
   :clojure.core.regex
   :clojure.core.atoms
   :clojure.core.volatiles
   :clojure.core.error-handling
   :clojure.core.macros
   :clojure.core.special-forms
   :clojure.core.printing
   :clojure.core.arrays
   :clojure.core.concurrency
   :clojure.core.agents
   :clojure.core.refs
   :clojure.core.dynamic-vars
   :clojure.core.namespaces
   :clojure.core.multimethods
   :clojure.core.protocols
   :clojure.core.records-types
   :clojure.core.type-system
   :clojure.core.threading
   :clojure.core.destructuring
   :clojure.core.uncategorized])

(defn escape-str
  "Escape a string for embedding in Clojure source."
  [s]
  (-> s
      (str/replace "\\" "\\\\")
      (str/replace "\"" "\\\"")
...
(defn emit-cljc
  "Generate parity.cljc from reference.edn. Standalone, minimal deps."
  [output-path]
  (let [ref (edn/read-string (slurp reference-file))
        ;; Only emit value tests (skip errors — they need try/catch)
        value-tests (remove :error ref)
        ;; Group by category
        by-cat (group-by :category value-tests)
        ;; Only include clojure.core.* categories (not clojure.core.reducers etc)
        ;; and skip host/scaling (not portable)
        by-cat (into {} (filter (fn [[k _]]
                                  (let [s (name k)]
                                    (and (str/starts-with? s "clojure.core.")
                                         (not (str/starts-with? s "clojure.core.reducers"))
                                         (not (str/starts-with? s "clojure.core.server"))
                                         (not (str/starts-with? s "clojure.core.protocols.clojure")))))
                                by-cat))
        ;; Order: known layers first, then everything else alphabetically
        known-set (set layer-order)
        ordered-cats (concat
                       (filter #(seq (get by-cat %)) layer-order)
                       (sort (remove known-set (keys by-cat))))]
...
;;
;; Standalone Clojure parity test. Runs on any implementation.
;; Needs only: def, defn, fn, if, do, let, =, +, str, println
;;
;; Drop this file into your implementation and run it.
;; ═══════════════════════════════════════════════════════════════════════

")
      ;; Minimal runner — no atoms, no try/catch, no macros
      (.write w "(def _pass 0)
(def _fail 0)
(def _layer nil)
(def _layer-pass 0)
...
(defn _report-layer []
  (if _layer
    (println (str \"  \" _layer \": \" _layer-pass \"/\" _layer-total))))
...
(defn _check [name result expected]
  (def _layer-total (+ _layer-total 1))
  (if (= (pr-str result) expected)
    (do (def _pass (+ _pass 1))
        (def _layer-pass (+ _layer-pass 1)))
    (do (def _fail (+ _fail 1))
        (println (str \"  FAIL \" name \": expected \" expected \", got \" (pr-str result))))))
...
")
      ...
      (doseq [cat ordered-cats]
        (let [tests (get by-cat cat)
              label (name cat)]
          (.write w (str "(_begin \"" label "\")\n"))
          (doseq [{:keys [expr it result]} tests]
            ;; Skip: unprintable, objects, infinite seqs, errors,
            ;; non-deterministic (rand, gensym, uuid, shuffle, time),
            ;; environment-dependent (thread bindings, classloaders)
            (when (and result
                       (not (str/starts-with? result "#object"))
                       (not (str/starts-with? result "#<"))
                       (not (str/starts-with? result "#error"))
                       (not (str/includes? result "..."))
                       (not (re-find #"gensym|rand|uuid|shuffle|thread-bind|Elapsed|class.?loader"
                                     (str/lower-case (str it))))
                       ;; Skip expressions with Java class refs
                       (not (re-find #"java\.|clojure\.lang\." expr)))
              (.write w (str "(_check \"" (escape-str it) "\" "
                             expr " \"" (escape-str result) "\")\n"))))
          (.write w "\n")))

      ;; Footer
      (.write w "(_report-layer)
(println)
(println (str _pass \"/\" (+ _pass _fail) \" pass\"))
(if (= _fail 0)
  (println \"All tests passed.\")
  (println (str _fail \" failures.\")))
"))

    (let [total (count (remove #(or (str/starts-with? (str (:result %)) "#object")
                                    (str/starts-with? (str (:result %)) "#<"))
                               value-tests))]
      (println (format "  Generated %s (%d tests)" output-path total)))))
...
(defn -main [& args]
  (let [args (vec args)
        ...
        scope  (cond
                 ...
                 :else                      :all)
        cmd (some #{"capture" "expand" "verify" "coverage" "cljc"} args)
        args (vec (remove #{"--quick" "--balanced" "--thorough" "--lang" "--contrib"
                            "capture" "expand" "verify" "coverage" "cljc"} args))
    ...
    (binding [*tier* tier]
      (cond
        ...
        (= cmd "cljc")    (emit-cljc (or (first (remove #(str/starts-with? % "--") args))
                                          "parity.cljc"))

        (or (= cmd "coverage") coverage-dir)
        (let [_ (when-not (and coverage-dir host-data)
                  (println "ERROR: --coverage requires --coverage <dir> --host-data <file.edn>")
                  (System/exit 1))
